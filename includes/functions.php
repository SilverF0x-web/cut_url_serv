...
function register_user($auth_data) {
    if (empty($auth_data) || !isset($auth_data['login']) || empty($auth_data['login'] || !isset($auth_data['pass']) || !isset($auth_data['pass_2']))) return false;
    ...
    if ($auth_data['pass'] !== $auth_data['pass_2']) {
...
function delete_link($id) {
    ...
    return db_query("DELETE FROM `links` WHERE `id` = $id;")->fetchAll();
}
